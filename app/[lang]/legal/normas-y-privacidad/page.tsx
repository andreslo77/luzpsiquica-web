// app/[lang]/legal/normas-y-privacidad/page.tsx
...
export default async function NormasPrivacidadPage({ params }: PageProps) {
  const p = await Promise.resolve(params);
  const lang = normalizeLang(p?.lang);
  const { NORMAS } = getLegalDocs(lang);

  const waDigits = (NORMAS.whatsappNumberRaw || "").replace(/[^\d]/g, "");
  const waMe = waDigits ? `https://wa.me/${waDigits}` : null;
  const mailto = NORMAS.email
    ? `mailto:${NORMAS.email}?subject=${encodeURIComponent(
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm" style={{ color: "rgba(183,183,199,0.95)", fontWeight: 700 }}>
              {lang === "en" ? "Version" : "Versión"}: {NORMAS.version}
              {NORMAS.owner ? ` · ${lang === "en" ? "Legal owner" : "Titular legal"}: ${NORMAS.owner}` : ""}
              {NORMAS.jurisdiction ? ` · ${lang === "en" ? "Jurisdiction" : "Jurisdicción"}: ${NORMAS.jurisdiction}` : ""}
            </p>
          </div>
              ...
              {s.title.includes("Contacto") || s.title.includes("Contact") ? (
                <div className="mt-4 flex flex-col gap-3">
                  ...
                  {waMe ? (
                    ...
                    >
                      WhatsApp: {NORMAS.whatsappNumberPretty}
                    </a>
                  ) : null}
                </div>
              ) : null}
